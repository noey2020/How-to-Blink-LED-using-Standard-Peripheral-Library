...
Within the purview of Cube HAL and Standard Peripheral Library(SPL), I noticed a lot of
deprecated defines and functions. So I had to choose which to discard and which to retain
while still leveraging code reuse. Pre-written code maybe working, reliable, and mature.
On the other hand, new techniques and data structures may be tested since we have no 
memory constraints like in the dinosaur pre-2000 times. See attached second draft verified
which handled the implicit declaration and assert errors and warnings that emanated from 
the first draft and finally solved.
